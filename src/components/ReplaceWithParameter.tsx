...
const ReplaceWithParameter = (props: IProps) => {
  props.text = props.text.replace(
    ...
    props.state.companyInfo.info.name
  );
  props.text = props.text.replace(
    /{{COMPANYINFO.ADDRESS}}/gim,
    props.state.companyInfo.info.address
  );
  props.text = props.text.replace(
    /{{COMPANYINFO.DISTRICH}}/gim,
    props.state.companyInfo.info.district
  );
  props.text = props.text.replace(
    /{{COMPANYINFO.CITY}}/gim,
    props.state.companyInfo.info.city
  );
  props.text = props.text.replace(
    /{{COMPANYINFO.COUNTRY}}/gim,
    props.state.companyInfo.info.country
  );
  props.text = props.text.replace(
    /{{COMPANYINFO.PHONE}}/gim,
    props.state.companyInfo.info.phone
  );
  props.text = props.text.replace(
    /{{COMPANYINFO.FAX}}/gim,
    props.state.companyInfo.info.fax
  );
  props.text = props.text.replace(
    /{{COMPANYINFO.WEBSITE}}/gim,
    props.state.companyInfo.info.webSite
  );
  props.text = props.text.replace(
    /{{COMPANYINFO.TAXOFFICE}}/gim,
    props.state.companyInfo.info.taxOffice
  );
  props.text = props.text.replace(
  ...
  props.text = props.text.replace(
    /{{COMPANYINFO.MERSISNUMBER}}/gim,
    props.state.companyInfo.info.mersisNumber
  );
  props.text = props.text.replace(
  ...
  if (props.state.bankInfo.list.length > 0) {
    const body = props.state.bankInfo.list.reduce(
      (content: string, item: BankInfoModel) => {
        return (content +=
          "<tr><td>" +
          item.bankName +
          "</td><td>" +
  ...
  if (props.state.logo.logoBase64 !== undefined) {
    props.text = props.text.replace(
      /{{LOGO}}/gim,
      '<img src="' + props.state.logo.logoBase64 + '" />'
    );
  } else {
    props.text = props.text.replace(/{{LOGO}}/gim, "");
  }

  if (props.state.signature.signatureBase64 !== undefined) {
    props.text = props.text.replace(
      /{{IMZA}}/gim,
      '<img src="' + props.state.signature.signatureBase64 + '" />'
    );
  } else {
